...
for ($z = 0; $z -lt $Profiles.Length; $z++){
	$Profile = $Profiles[$z]
	$TopPSScriptRoot= sl -Path $PsscriptRoot\Profiles\$profile
	...
	$docker = dir "$PsScriptRoot\Profiles\$profile\dockerfile*" -recurse -Force |% {$_ | select-string -pattern " "} 
	$dockermassive = $docker -replace "(.+)[a-z]\:\d+:"
	$check[$z] = $dockermassive[0]
	if ($dockermassive[0] -notmatch "AS"){
		$dockermassive[0] = $dockermassive[0] + " AS $Profile"
		}
...
$summdocker | out-file -FilePath $PsScriptRoot\Release\$newname\dockerfile -Encoding utf8